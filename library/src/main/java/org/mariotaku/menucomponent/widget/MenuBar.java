...
public class MenuBar extends LinearLayout implements MenuItem.OnMenuItemClickListener {
	...
	private final List<PopupMenu> mPopupMenus = new ArrayList<PopupMenu>();
	...
	public MenuBar(final Context context, final AttributeSet attrs) {
		...
		final TypedArray a = context
				.obtainStyledAttributes(attrs, new int[] { android.R.attr.max, android.R.attr.src });
		mMaxItemsShown = a.getInt(0, getResources().getInteger(R.integer.mc__max_action_buttons));
		final int resourceId = a.getResourceId(1, 0);
		if (resourceId != 0) {
			inflate(resourceId);
			show();
		}
	...
	protected void onDetachedFromWindow() {
		for (PopupMenu popupMenu : mPopupMenus) {
	...
	private View createMoreOverflowButton(final ArrayList<MenuItem> itemsNotShowing) {
		...
		view.setOnTouchListener(new MenuItemDragOpenListener(view, itemsNotShowing, this));
		view.setOnClickListener(new MoreOverflowOnClickListener(view, itemsNotShowing, this));
		return view;
	}
	...
	private View createViewForMenuItem(final MenuItem item) {
		...
		if (actionView != null) {
			...
			if (isEnabled && item.hasSubMenu()) {
				view.setOnTouchListener(new MenuItemDragOpenListener(view, item.getSubMenu(), this));
			}
			...
			final TextView titleView = (TextView) view.findViewById(R.id.mc__item_title);
			final ImageView iconView = (ImageView) view.findViewById(R.id.mc__item_icon);
	...
	private void registerPopupMenu(final PopupMenu popupMenu) {
		if (popupMenu == null) return;
		mPopupMenus.add(popupMenu);
	}
	...
	private static boolean hasVisibleItems(final Menu menu) {
	...
	private static class ActionViewOnClickListener implements OnClickListener {
		...
		public void onClick(final View actionView) {
			...
			if (menuItem.hasSubMenu()) {
				...
				if (!hasVisibleItems(subMenu)) return;
				...
				popupMenu.show();
				menuBar.registerPopupMenu(popupMenu);
			} else {
	...
	private static class MoreOverflowOnClickListener implements OnClickListener {
		...
		private MoreOverflowOnClickListener(final View actionView, final ArrayList<MenuItem> menuItems,
		...
		public void onClick(final View view) {
			...
			final PopupMenu popupMenu = PopupMenu.getInstance(view.getContext(), view);
			popupMenu.setOnMenuItemClickListener(menuBar);
	...
	private static class MenuItemDragOpenListener implements View.OnTouchListener {

		private final PopupMenu popupMenu;
		private final OnTouchListener dragToOpenListener;

		private MenuItemDragOpenListener(final View actionView, final ArrayList<MenuItem> menuItems,
				final MenuBar menuBar) {
			this(actionView, MenuUtils.createMenu(menuBar.getContext(), menuItems), menuBar);
		}

		private MenuItemDragOpenListener(final View actionView, final Menu menu, final MenuBar menuBar) {
			popupMenu = PopupMenu.getInstance(actionView.getContext(), actionView);
			popupMenu.setOnMenuItemClickListener(menuBar);
		...
		public boolean onTouch(final View v, final MotionEvent event) {
			if (dragToOpenListener == null) return false;
			return dragToOpenListener.onTouch(v, event);
		}
	...
	private static class OnActionItemLongClickListener implements OnLongClickListener {
		...
		public boolean onLongClick(final View v) {
			...
			final boolean showText = (MenuUtils.getShowAsActionFlags(item) & MenuItem.SHOW_AS_ACTION_WITH_TEXT) != 0;
			if (item.getIcon() == null || showText) return false;
